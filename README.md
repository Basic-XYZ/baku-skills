...
| [**baku-illustrations（Baku 正文配图）**](#baku-illustrationsbaku-正文配图) | 为中文文章生成 Baku 程序员个人 IP 风格的正文配图和 shot list | [SKILL.md](./baku-illustrations/SKILL.md) |
...
把 `--skill` 换成要装的目录名，比如 `boss-job-hunter` 或 `baku-coding-discipline`。安装器应该只安装包含 `SKILL.md` 的那个 skill 目录，不要把整个仓库当成一个 skill。
...
这个 skill 面向中文文章、博客、知识库、教程和复盘的正文配图。它借鉴 `ian-xiaohei-illustrations` 的结构，也参考了 Esther 把小黑迁移成“不二”的方式；但 Baku 版本把角色、颜色、线条质感、prompt 模板和 QA 标准全部换成了自己的程序员个人 IP。
...
- 生成 16:9 横版暖纸手绘正文配图
- 使用 Baku 程序员 IP 参与核心动作，表达证据链、流程、系统局部、前后对比和方法分层
...
新增 skill 时，优先保持这个结构。新增目录和 frontmatter `name` 默认使用 `baku-` 前缀；`boss-job-hunter` 和 `codex-history-recovery` 是早期例外，保持原名不改。不要把多个 skill 混在一个目录里，也不要把整个仓库包装成一个 skill。
